  ...
  return (
    <Card className="overflow-hidden [&_*]:duration-0 [&_td]:transition-none [&_th]:transition-none [&_tr]:transition-none [&>div]:scrollbar-thin [&>div]:scrollbar-track-transparent [&>div]:scrollbar-thumb-slate-300/80 hover:[&>div]:scrollbar-thumb-slate-400 dark:[&>div]:scrollbar-thumb-slate-700/80 dark:hover:[&>div]:scrollbar-thumb-slate-600">
      <Table className="min-w-[1180px] border-separate border-spacing-0">
        ...
        <TableHeader>
          <TableRow>
            <TableHead className="sticky left-0 z-40 border-r border-border bg-card px-3 after:pointer-events-none after:absolute after:inset-y-0 after:right-0 after:z-40 after:w-px after:bg-border">
              名称
            </TableHead>
            ...
            <TableHead className="whitespace-nowrap">Ping</TableHead>
        ...
        <TableBody>
          {nodes.map(n => {
            ...
            const ping = latency ? getLatency(latency, n.uuid, 'ping', '1h') : null
            const tcp = latency ? getLatency(latency, n.uuid, 'tcp_ping', '1h') : null
            const loss = Math.max(ping?.lossRate ?? 0, tcp?.lossRate ?? 0)
            return (
              <TableRow
                key={n.uuid}
              ...
              >
                <TableCell className="sticky left-0 z-40 border-r border-border bg-card px-3 font-medium after:pointer-events-none after:absolute after:inset-y-0 after:right-0 after:z-40 after:w-px after:bg-border group-hover:bg-muted group-hover:after:bg-border">
                ...
                <TableCell className="font-mono">{latencyCell(ping?.avg)}</TableCell>
